...
pub(crate) fn validate_scalar(
    ...
    let result = match tag {
        super::schema_view::SchemaTag::None => validate_none(input),
        super::schema_view::SchemaTag::Bool => validate_bool(input, strict, profile),
        super::schema_view::SchemaTag::Integer => match schema.integer() {
            Ok((target, constraints)) => validate_integer(input, options, target, &constraints),
            Err(error) => Err(error),
        },
        super::schema_view::SchemaTag::String => match schema.string() {
        ...
        super::schema_view::SchemaTag::Decimal => match schema {
            SchemaRef::Owned(Schema::Decimal(constraints)) => {
                validate_decimal(input, options, constraints)
            }
            SchemaRef::Static(_) => {
                validate_decimal(input, options, &DecimalConstraints::default())
            }
            _ => return None,
        },
        super::schema_view::SchemaTag::Fraction => match schema {
            SchemaRef::Owned(Schema::Fraction(constraints)) => {
                validate_fraction(input, options, constraints)
            }
            _ => return None,
        },
        super::schema_view::SchemaTag::Complex => match schema {
            SchemaRef::Owned(Schema::Complex(constraints)) => {
                validate_complex(input, strict, profile, constraints)
            }
            _ => return None,
        },
        _ => return None,
