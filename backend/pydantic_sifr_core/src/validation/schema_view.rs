...
use super::{
    ...
    StringConstraints, StringPattern, ValidationError,
};
...
use static_values::{
    bool_value, field, field_value, list, optional_big_int, optional_f64, optional_usize, record,
    schema_error, string, usize_value,
};
...
impl<'schema> ModelRef<'schema> {
    ...
    pub fn root_model(self) -> Result<bool, ValidationError> {
        match self {
            Self::Owned(model) => Ok(model.root_model),
            Self::Static(model) => model.flag("root_model"),
        }
    }
}
