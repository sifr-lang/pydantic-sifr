...
| `api/type_adapter` | `TypeAdapter[T]` | adapted | One prepared Core Schema owns validation, serialization, and JSON Schema generation for `T`. | `backend/pydantic_sifr_core/tests/type_adapter.rs` |
...
| `api/json_schema` | JSON Schema | adapted | Draft 2020-12 documents are generated from the same Core Schema with deterministic `$defs`, aliases, constraints, and mode-specific representations. | `backend/pydantic_sifr_core/tests/json_schema_dialect.rs` |
