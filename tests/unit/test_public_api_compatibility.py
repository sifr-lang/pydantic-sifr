...
LEDGER = ROOT / "tests/compatibility/public_api.toml"
...
class PublicApiCompatibilityTest(unittest.TestCase):
    def test_machine_matrix_is_total_and_evidence_backed(self) -> None:
        ...
        self.assertEqual(payload["scope"], "public_api")
    ...
    def test_selected_upstream_families_are_anchored(self) -> None:
        anchors = tomllib.loads(ANCHORS.read_text(encoding="utf-8"))["anchor"]
        fixtures = {
            row["fixture"]
            for row in anchors
            if row["fixture"] in PUBLIC_API_ANCHORED
        }
        self.assertEqual(fixtures, PUBLIC_API_ANCHORED)
