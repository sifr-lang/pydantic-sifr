...
LEDGER = ROOT / "tests/compatibility/ps8.toml"
...
class Ps8CompatibilityTest(unittest.TestCase):
    def test_ledger_is_total_for_the_delivered_serialization_bound(self) -> None:
        ...
        self.assertEqual(payload["milestone"], "ps_8")
        rows = payload["family"]
    ...
    def test_delivered_families_have_ps8_upstream_anchors(self) -> None:
        anchors = tomllib.loads(ANCHORS.read_text(encoding="utf-8"))["anchor"]
        fixtures = {row["fixture"] for row in anchors if row["milestone"] == "ps_8"}
