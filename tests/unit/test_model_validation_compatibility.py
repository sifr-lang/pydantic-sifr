...
LEDGER = ROOT / "tests/compatibility/model_validation.toml"
...
class ModelValidationCompatibilityTest(unittest.TestCase):
    ...
    def test_every_upstream_anchor_maps_to_a_ledger_family(self) -> None:
        anchors = tomllib.loads(ANCHORS.read_text(encoding="utf-8"))["anchor"]
        fixtures = {row["fixture"] for row in anchors if row["fixture"] in REQUIRED}
